...
from mmcv.ops import nms_rotated, batched_nms_rotated
...
def multiclass_rnms(multi_bboxes,
    ...
    labels = torch.nonzero(valid_mask)[:, 1]
...
def aug_multiclass_rnms(global_bboxes,
    ...
    nms_thr = nms_cfg_.pop('iou_threshold', 0.05)
    ...
    for cls in classes:
        ...
        _bboxes_with_score = cls_bboxes[_inds, :]
        _boxes = _bboxes_with_score[:,:-1]
        _scores = _bboxes_with_score[:,-1]
        

        iou_thr = nms_thr[cls] if hasattr(nms_thr, '__getitem__') else nms_thr

        cls_dets, _ = nms_op(_boxes, **nms_cfg_, scores=_scores, iou_threshold=iou_thr)
